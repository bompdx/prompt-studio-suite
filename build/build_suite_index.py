...
cards = "\n".join(
    f'''<a class="studio-card" href="{html.escape(s['slug'])}-prompt-studio.html" style="--studio:{html.escape(s['accent'])};--studio2:{html.escape(s['accent2'])}">
      ...
      <span class="card-kicker">{html.escape(s['kicker'])}</span>
      <strong>{html.escape(s['title'].replace(' Prompt Studio', ''))}</strong>
      <span class="card-copy">{html.escape(s['subtitle'].split('. ')[0])}.</span>
      <span class="card-meta"><b>{len(s['workflows'])}</b> workflows <i>•</i> <b>{len(s['advisors'])}</b> advisors</span>
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .hero {{ display:grid; grid-template-columns:minmax(0,1.25fr) minmax(280px,.75fr); gap:40px; margin-top:18px; padding:clamp(30px,6vw,72px); border:1px solid var(--line); border-radius:30px; background:var(--panel); box-shadow:var(--shadow); overflow:hidden; position:relative; }}
    ...
    .suite-note {{ position:relative; z-index:1; align-self:center; padding:22px; border:1px solid var(--line); border-radius:20px; background:color-mix(in srgb,var(--paper) 55%,var(--panel)); }}
    .suite-note b {{ display:block; font-size:2.6rem; letter-spacing:-.05em; }} .suite-note p {{ margin:7px 0 0; color:var(--muted); font-weight:650; }}
    .section-head {{ display:flex; justify-content:space-between; align-items:end; gap:18px; margin:42px 0 16px; }} h2 {{ margin:0; font-size:clamp(1.8rem,4vw,2.8rem); letter-spacing:-.04em; }} .section-head p {{ max-width:520px; margin:0; color:var(--muted); font-weight:620; }}
    .grid {{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:14px; }}
    .studio-card {{ position:relative; min-height:270px; display:grid; align-content:start; gap:9px; padding:23px; border:1px solid var(--line); border-radius:20px; background:var(--panel); overflow:hidden; transition:transform .2s ease,border-color .2s ease,box-shadow .2s ease; }}
    ...
    .card-index,.card-kicker {{ font:850 .7rem/1 ui-monospace,SFMono-Regular,Menlo,monospace; text-transform:uppercase; letter-spacing:.09em; }} .card-index {{ color:var(--studio); }} .card-kicker {{ margin-top:13px; color:var(--muted); }}
    .studio-card strong {{ max-width:520px; font-size:clamp(1.6rem,3vw,2.45rem); line-height:1; letter-spacing:-.045em; }} .card-copy {{ color:var(--muted); font-weight:620; line-height:1.55; }}
    .card-meta {{ margin-top:auto; padding-top:14px; color:var(--muted); font:750 .78rem/1.3 ui-monospace,SFMono-Regular,Menlo,monospace; }} .card-meta i {{ margin:0 5px; font-style:normal; }} .card-link {{ margin-top:8px; color:var(--studio); font-weight:900; }}
...
<body><div class="shell">
  ...
  <main><section class="hero"><div><p class="kicker">Ten focused prompt workbenches</p><h1>Pick the work. Build the prompt.</h1><p class="intro">Each studio turns a real business job into a guided form and a complete prompt. Add your context once, choose the artifact or decision you need, then take the finished prompt into ChatGPT, Claude, or the assistant you already use.</p></div><aside class="suite-note"><p class="kicker">Inside the collection</p><b>{sum(len(s['workflows']) for s in studios)}</b><p>guided workflows across {len(studios)} markets, plus {sum(len(s['advisors']) for s in studios)} ongoing advisor modes.</p></aside></section>
  <div class="section-head"><div><p class="kicker">The collection</p><h2>Choose your studio</h2></div><p>Every studio is a standalone file. Your saved profile and prompt manual stay in your browser.</p></div><section class="grid" aria-label="Prompt studios">{cards}</section></main>
  <footer><span>Prompt Studio Suite</span><span>Local-first tools for focused work.</span></footer>
</div><script>
